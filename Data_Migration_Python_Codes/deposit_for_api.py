...
def mapping_funcV2(invoice_from_qb):
    ...
    chart_of_accounts_from_zohofout = open('coa_details_PR.json')
    chart_of_accounts_from_zoho = json.load(chart_of_accounts_from_zohofout)
    ...
    if "DocNumber" in invoice_from_qb:
        ...
        mapping_dict["entry_number"] = f"DEPOSIT_{journal_entry_count}-{extracted_year}"
        mapping_dict["journal_number_prefix"] = f"DEPOSIT_{journal_entry_count}-{extracted_year}-"
        mapping_dict["journal_number_suffix"] = f"{journal_entry_count}"
    ...
    if "DepositToAccountRef" in invoice_from_qb:
        ...
        first_line_item["amount"] = invoice_from_qb["TotalAmt"]
        first_line_item["debit_or_credit"] = 'debit'
        first_line_item["account_id"] = chart_of_accounts_from_zoho[invoice_from_qb["DepositToAccountRef"]["name"]]
    ...
    if "Line" in invoice_from_qb:
        ...
        for item in qb_line_items:
            ...
            if detail_type == "DepositLineDetail":
                ...
                if "EntityRef" in item["DepositLineDetail"]:
                    my_notes += f' Name: {item["DepositLineDetail"]["Entity"]["name"]}, Type: {item["DepositLineDetail"]["Entity"]["type"]}'
                

                found_class_name = False
                if "ClassRef" in item["DepositLineDetail"]:
                    class_name = item["DepositLineDetail"]["ClassRef"]["name"]
                    found_class_name = True
                
                found_department_name = False
                if "DepartmentRef" in item["DepositLineDetail"]:
                    department_name = item["DepositLineDetail"]["DepartmentRef"]["name"]
                    found_department_name = True
                
                found_payment_method = False
                if "PaymentMethodRef" in item["DepositLineDetail"]:
                    payment_method = item["DepositLineDetail"]["PaymentMethodRef"]["name"]
                    found_payment_method = True
                
                found_check_num = False
                if "DepartmentRef" in item["DepositLineDetail"]:
                    check_num = item["DepositLineDetail"]["DepartmentRef"]["name"]
                    found_check_num = True
                ...
                if found_payment_method:
                    extra_description = f"Payment Method: {payment_method} "
                if found_check_num:
                    extra_description = f"CheckNum: {check_num} "
                ...
                # posting_type = item["DepositLineDetail"]["PostingType"]
                ...
                account_name = item["DepositLineDetail"]["AccountRef"]["name"]
                splitted_name_list = account_name.split(':')
                if len(splitted_name_list) == 1:
                    # No colon
                    account_name = splitted_name_list[0]
...
def generate_post_json():
    ...
    file_name = '../data_from_qb_deposit_pr.json'
    ...
    with open("mapped_deposit_PR.json", "w") as outfile:
        outfile.write(mapped_json)
...
def post_request_zoho():
    ...
    response_save_file = open("saved_deposit_PR.txt","a")
