...
def get_items_and_write_to_file():
    ...
    file_name = 'data_from_qb_taxrate_PR.json'
    # file_name = 'data_from_qb_2017_03_01_to_2023_01_31_taxrate.json'
    fp = open(file_name,'a')
    startPosition = 1
    no_of_objects_fetched = 0
    flag = True
    while flag == True:
        selectQuery = f"select * from TaxRate startPosition {startPosition}"
        ...
        if r.status_code == 200:
            ...
            if "QueryResponse" in parsed_data:
                if "TaxRate" in parsed_data["QueryResponse"]:
                    for element in parsed_data["QueryResponse"]["TaxRate"]:
...
def generate_post_json():
    ...
    # file_name = 'all_fetched_data_from_qb.json'
    file_name = 'data_from_qb_taxrate.json'
