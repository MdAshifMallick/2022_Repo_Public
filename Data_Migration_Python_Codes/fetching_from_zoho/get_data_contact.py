...
def get_contacts_and_write_to_file():
    ...
    file_name = 'data_from_zoho_contacts_geo.json'
    fp = open(file_name,'a')
    no_of_objects_fetched = 0
    flag = True
    page = 1
    while flag == True:
        getURL = f"https://books.zoho.com/api/v3/contacts?page={page}&organization_id={organization_id}"
        ...
        if r.status_code == 200:
            ...
            if parsed_data["code"] == 0:
                contacts_list = parsed_data["contacts"]
...
get_contacts_and_write_to_file()
