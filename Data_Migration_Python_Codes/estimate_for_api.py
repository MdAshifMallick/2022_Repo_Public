...
def mapping_func(invoice_from_qb):
    ...
    if "DocNumber" in invoice_from_qb:
        ...
        mapping_dict["estimate_number"] = invoice_from_qb["DocNumber"]
    else:
        mapping_dict["estimate_number"] = null_value
    ...
    # cf_id_3_dict = dict()
    # cf_id_3_dict["label"] = "Expiration Date"
    # if "ExpirationDate" in invoice_from_qb:
    #     cf_id_3_dict["value"] = invoice_from_qb["ExpirationDate"]
    # else:
    ...
    # cf_id_5_dict = dict()
    # cf_id_5_dict["label"] = "Class"
    # if "ClassRef" in invoice_from_qb:
    #     cf_id_5_dict["value"] = invoice_from_qb["ClassRef"]["name"]
    # else:
    #     cf_id_5_dict["value"] = null_value

    # cf_id_5_dict = dict()
    # cf_id_5_dict["label"] = "BillEmail"
    # if "BillEmail" in invoice_from_qb:
    #     cf_id_5_dict["value"] = invoice_from_qb["BillEmail"]["Address"]
    # else:
    #     cf_id_5_dict["value"] = null_value
    
    # cf_id_6_dict = dict()
    # cf_id_6_dict["label"] = "BillEmailCc"
    # if "BillEmailCc" in invoice_from_qb:
    #     cf_id_6_dict["value"] = invoice_from_qb["BillEmailCc"]["Address"]
    # else:
    #     cf_id_6_dict["value"] = null_value
    ...
    # if cf_id_3_dict["value"]:
    #     custom_field_list.append(cf_id_3_dict)
    # custom_field_list.append(cf_id_4_dict)
    # custom_field_list.append(cf_id_5_dict)
    # custom_field_list.append(cf_id_6_dict)
    ...
    if not mapping_dict["estimate_number"]:
        # bill_number is null
        ...
        mapping_dict["estimate_number"] = f"{vendor_name.split(' ')[0]}{count_dict[mapping_dict['customer_id']]}"
    ...
    if "DiscountAmt" in invoice_from_qb:
        mapping_dict["discount"] = invoice_from_qb["DiscountAmt"]
...
def generate_post_json():
    ...
    # file_name = '../data_from_qb_2013_04_01_to_2017_06_30_estimate.json'
    file_name = '../data_from_qb_estimate_pr.json'
    ...
    # mapped_data = dict()
    # mapped_data["Estimate"] = my_item_list

    print(len(my_item_list))
    mapped_json = json.dumps(my_item_list,indent=4)

    with open("mapped_estm_PR.json", "w") as outfile:
        outfile.write(mapped_json)
...
def post_request_zoho():
    ...
    access_token = "1000.2d69b783a8f4276498bf5113d7b0d915.c8390cbf85afb948713f2a1190fa4642"
    refresh_token = "1000.a8419b8e9c884a0690b4e12f15369390.0f1cfb20e799e354a8cb8e69d607af13"
    # expires_in = parsed_data["expires_in"]

    organization_id = '60019790139'
    post_url = f'https://books.zoho.in/api/v3/estimates?organization_id={organization_id}'

    f = open('mapped_estm_PR.json')
    # f = open('failed_mapped.json')

    parsed_data2 = json.load(f)
    # items_list = parsed_data2["Estimate"]
    ...
    response_save_file = open("saved_estimate_id_PR.txt","a")
    ...
    for item in items_list:
        while True:
            print(f'Processing for estimate_number: {item["estimate_number"]}||{item["date"]}||{item["customer_id"]}....')
            ...
            if parsed_data3["code"] == 57:
                ...
                # response_save_file.write(f'{parsed_data3["invoice"]["invoice_id"]} : {item["estimate_number"]}\n')
                response_save_file.write(f'{parsed_data3["estimate"]["estimate_id"]} : {id_from_qb}\n')
                ...
                """
                old_estimate_number = item["estimate_number"]
                new_estimate_number = for_duplicate_invoice_number(item["customer_id"],old_estimate_number)
                if not new_estimate_number:
                    # if new_estimate_number is EMPTY STRING
                    response_save_file.write(f'Failed to PUSH : {id_from_qb} -> msg is {parsed_data3["message"]} ## ESTIMATE_NUMBER {old_estimate_number} to NULL String returned\n')
                    break

                item["estimate_number"] = new_estimate_number
                response_save_file.write(f'Failed to PUSH : {id_from_qb} -> msg is {parsed_data3["message"]} ## ESTIMATE_NUMBER {old_estimate_number} to {item["estimate_number"]}\n')
                response_save_file.flush()
            else:
                print(f'ERROR occurred: {id_from_qb}||{item["estimate_number"]}||{item["date"]}||{item["customer_id"]} was not able to be pushed')
        ...
        response_save_file.flush()
        # print(f'Successfully created creditnote named: {item["estimate_number"]}||{item["date"]}||{item["customer_id"]}')
