...
my_invoice_id_list = list()

def mapping_func(invoice_from_qb):

    global my_invoice_id_list
    ...
    invoice_qb_id_against_invoice_id_from_zohofout = open('data_from_zoho/invoice_qb_id_against_invoice_id_from_zoho_PR.json')
    ...
    #     "2581":"127706000006158221",
    #     "2582":"127706000006158205",
    #     "5081":"127706000006154908",
    #     "946":"127706000001742741",
    #     "232":"127706000001747499",
    #     "640":"127706000006165233",
    #     "2317":"127706000006150332",
    #     "636":"127706000006165179",
    #     "243":"127706000001747481",
    #     "628":"127706000006157891",
    #     "629":"127706000006179423",
    #     "2327":"127706000006158315"
    # }
    ...
    mapping_dict["customer_id"] = contacts_from_zoho[invoice_from_qb["CustomerRef"]["name"]]
    mapping_dict["id_from_qb"] = invoice_from_qb["Id"]

    mapping_dict["payment_mode"] = null_value
    if "PaymentMethodRef" in invoice_from_qb:
        mapping_dict["payment_mode"] = payment_method_dict[invoice_from_qb["PaymentMethodRef"]["value"]][0]

    mapping_dict["account_id"] = None
    if "DepositToAccountRef" in invoice_from_qb:
        mapping_dict["account_id"] = chart_of_accounts_from_zoho[account_id_dict[invoice_from_qb["DepositToAccountRef"]["value"]]]

    if not mapping_dict["account_id"]:
    ...
    for line_item in invoice_from_qb["Line"]:
        ...
        if len(linked_txn) == 1:
            if linked_txn[0]["TxnType"] == "Invoice":
                invoice_dict = dict()
                ...
                if invoice_id:
                    ...
                    my_invoice_id_list.append([invoice_from_qb["Id"],linked_txn[0]["TxnId"]])
                    invoice_dict["invoice_id"] = None
                    return None
    ...
    mapping_dict["invoices"] = invoice_list
    mapping_dict["amount"] = amount_applied


    return mapping_dict

def mapping_func_customer_advance(invoice_from_qb):
...
def generate_post_json():
    ...
    # file_name = '../data_from_qb_before_payment.json'
    file_name = '../skipped_ip_PR.json'
    ...
    for item in parsed_data:
        ...
        mapped_customer = mapping_func_customer_advance(item)
    ...
    with open("mapped_inv_payment_adv_PR.json", "w") as outfile:
        outfile.write(mapped_json)

    print(len(skipped_list))
    # mapped_json = json.dumps(skipped_list,indent=4)
    # with open("skipped_ip_PR.json", "w") as outfile:
    #     outfile.write(mapped_json)
    ...
    print(len(my_invoice_id_list))
    # mapped_json = json.dumps(my_invoice_id_list,indent=4)
    # with open("my_invoice_id_list.json", "w") as outfile:
    #     outfile.write(mapped_json)
...
def post_request_zoho_payment():
    ...
    post_url = f'https://books.zoho.in/api/v3/customerpayments?organization_id={organization_id}'

    # f = open('mapped.json')
    # f = open('failed_mapped.json')
    # f = open('to_again_mapped.json')
    # f = open('payment_mapped.json')
    f = open('mapped_inv_payment_adv_PR.json')
    ...
    # response_save_dict = dict()
    response_save_file = open("saved_payment_inv_PR.txt","a")
    error_response_save_file = open("error_console_for_failed_invoice.txt","a")
    ...
    for item in items_list:
        ...
        while True:
            ...
            if parsed_data3["code"] == 57:
                ...
                # response_save_dict[f'{parsed_data3["contact"]["contact_id"]}'] = item["name"]
                # response_save_file.write(f'{parsed_data3["invoice"]["invoice_id"]} : {item["invoice_number"]}\n')
                response_save_file.write(f'{parsed_data3["payment"]["payment_id"]} : {id_from_qb}\n')

                print(f'{id_from_qb} is successfully pushed')
                # It means success
                break
            else:
                print(f'ERROR occurred: {id_from_qb}||{item["id_from_qb"]} was not able to be pushed')
                print(f'error code returned in json is {parsed_data3["code"]} and msg is {parsed_data3["message"]}')
                response_save_file.write(f'Failed to PUSH : {id_from_qb} -> msg is {parsed_data3["message"]}\n')   
                break
